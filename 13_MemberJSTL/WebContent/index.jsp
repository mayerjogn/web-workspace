...
<body>
	...
	<ul>
		...
		<%-- 
			회원가입 : 아이디, 비밀번호, 이름, 주소 입력받아서
		-> RegisterServlet / post 방식으로
		-> session에 데이터 바인딩하기
		...
		<%--
			...
			-> 세션에 데이터 바인딩 -> views/login_result.jsp로 (정보 출력)
		 --%>		
	...
	<%}else{%>
		...
		<%--
			회원검색 : 검색할 아이디를 입력받아서
			->FindMemberServlet -> 검색 성공하면 views/find_ok.jsp로 (정보 출력)
								->     실패하면 views/find_fail.jsp로 
		 --%>
		...
		<%-- 회원 정보 수정 : 로그인한 정보가 기재된 상태로 시작
						  ID 값은 변경불가능하게 처리
						  
		-> updateServlet / post 방식으로		
