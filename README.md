...
```java
<%= request.getParameter("name")%> 원래 표현법

${param.name} 이런식으로 간결하게 변경
```
...
- applicationScop : Applicaion 영역 객체에 접근
- param : 전달된 파라미터 값을 받아올 때 사용
- paramValues : 전달된 파라미터들을 배열로 받아올 때 사용

### EL 연산자

- 덧셈 뺄셈 곱셈 기호는 같음 나눗셈은 div연산자를 사용 나머지는 mod연산자 사용
- and or ! 는 and or not으로변경됨
- < 은 lt로 > 는 gt로 그 외는 아래 확인
...
- 커스텀 액션 태그 : 별도의 라이브러르 설치 필요, 라이브러리 선언에 맞는 접두어가 붙음 c : 태그
...
- JavaScript를 이용하여 비동기식으로 클라이언트와 서버가 데이터(XML)를 주고받는(통신)방식이다.

- 데이터 형식은 XML 뿐만 아니라 Text, HTML, **JSON**, CSV 등 다양한 형식을 사용 가능하다.

### 동기식 VS 비동기식

- **동기식 데이터 통신** : 클라이언트가 서버로 데이터를 요청하면 응답이 올때까지 다른 작업은 대기한다.(요청된 데이터 작업 우선)

- **비동기식 데이터 통신** : 클라이언트가 서버로 데이터 요청 후 응답을 기다리지 않고 다른 작업을 수행 가능하다, 추후 응답이 오면 응답에 관련된 작업이 진행된다. (따로따로 진행함)

### 특징

- 전체 페이지를 갱신하지 않고 일부분만 업데이트가 가능하며 사용자에게 즉각적인 반응과 풍부한 UI 경험을 제공한다.

- Javascript 또는 jQuery 방식으로 구현한다.
...
- 페이지 내 복잡도가 증가하여 에러 발생 시 디버깅이 어려워짐
...
3. open() 메소드를 호출하여 요청 방식, 대상(서버), 동기/비동기 지정
4. send() 메소드를 호출하여 대상(서버)에 전송

## jQuery 방식

- jQuery에 내장되어 있는 Ajax 통신을 위한 가장 기본적인 함수 이용함
...
- url : 요청(request) 데이트를 전송할 URL
- type : Http 요청 방식 지정(GET / POST)
- data : 서버로 전송할 요청 Parameter 설정
- datatype : 서버의 응답(response) 데이터의 형식(xml, text, json, html 등) 지정, 미 작성 시 자동으로 판단하여 지정됨
- success(data) : ajax 통신 성공 시 호출되는 함수를 지정하는것 매개변수로 응답 데이터(data)를 받음
- error : ajax 통신 실패 시 호출되는 함수를 지정
- complete : ajax 통신 성공여부와 관계없이 통신 완료 후 실행되는 함수를 지정
- async : 비동기(true) / 동기(false) 지정

### 장점

- 코드 길이가 감소하여 JavaScript 방식보다 구현 방법이 더 간단해짐
- 직관적이며 다양한 방법의 코딩 가능
- 크로스 브라우저 처리를 jQuery가 자동으로 해결해줌
...
- Ajax 통신에서 Object 타입의 데이터 전송 시 XNL 대비 용량이 작고 속도가 빠르다. (경량 데이터 교환 방식)
- 간단한 포맷을 가지고 있어 이해하기 쉬움
- 순수 TEXT 기반으로 TEXT 형식이 구조화 되어 있으며 대부분의 프로그래밍 언어에서 JSON 포맷 데이터를 핸들링 할 수 있는 라이브러리를 제공해 시스템간 객체 교환에 용이함
