...
public class ProductServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		// 2. 세션에 바인딩된 값이 있다면 그 값을 찾아온다. 로그인 서블릿에서 가져온거임
		MemberVO vo = (MemberVO) session.getAttribute("vo"); // addCast로 형 변환
		
		if (vo != null) { // 로그인 성공한 상태
			out.println("VO :: " + vo);
			out.println("<hr>");
			
			session.setAttribute("PRODUCT","NOTBOOK");
